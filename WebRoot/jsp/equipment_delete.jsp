...
function check(form2){
    ...
    if(checkCount == 0) {  
        ...
        if(form2.sssl.value==""){
		     alert("请输入删除数目!");form2.sssl.focus();return false;
	    }else{
	         form2.action="../servlet/EquipmentDel";
             return false;
        } 
    } 
}
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
  ...
  <tr>
    <td valign="top"><table width="100%" border="0" cellpadding="0" cellspacing="0" id="row_1" >
      <tr>
        <td class="left_bt3"><b>当前位置：器材管理&gt;&gt;删除器材</b></td>
      </tr>
      <tr>
...
<table id="rounded-corner">
    <thead>
    	<tr>
        	...
        	<th scope="col" class="rounded" align="center">器材数量</th>
     ...
     <tfoot>
    	...
    	<td>&nbsp;</td>
    	<td>&nbsp;</td>
        <td align="center">删除数量：<input name="sssl" type="text"></td>
    	<td>&nbsp;</td>
    	<td>&nbsp;</td>
    	<td class="rounded-foot-left">
        	<input name="Save" type="submit" class="btn_red" value="删除器材" onClick="check(form2)">
            </td>
        </tr>
        <tr>
    		...
    		<c:forEach var="item" items="${FindEquipment}"  varStatus="status">
            	<tr>
            		<td> <input type="checkbox" name="EquipmentDel" value="${status.index}" /></td>
   					...
   					<td> <c:out value="${item.compensation}/个"/></td>
   					<td> <c:out value="${item.eRentFees}/个"></c:out></td>
