...
function check(form2){
	var checked=true;
    var id= document.getElementsByName("EquipmentUpdate");
    var checkCount = 0;
    ...
    if(checkCount == 0) {  
        ...
        form2.action="../servlet/EquipmentUpdatetran";
        return false;  
    } 
}
</script>
...
<table id="rounded-corner">
    <thead>
    	<tr>
        	...
        	<th scope="col" class="rounded">器材数目</th>
     ...
     <tfoot>
        <tr>
        	...
        	<input name="Save" type="submit" class="btn_blue" value="更新器材" onClick="check(form2)">
            </td>
        </tr>
    	<tr>
    ...
    <tbody>
          ...
          <c:if test="${FindEquipment!=null}" var="flag" scope="page">
    		<c:forEach var="item" items="${FindEquipment}"  varStatus="status">
            	<tr>
            		<td> <input type="checkbox" name="EquipmentUpdate" value="${status.index}" /></td>
